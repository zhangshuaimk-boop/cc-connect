...
- Feishu/Lark projects now pass matching `lark-cli` bot credentials and tenant
  access tokens into agent subprocesses by default; set
  `inject_lark_cli_credentials = false` per project to keep using the active
  local profile.
