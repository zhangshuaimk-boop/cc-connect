...
func TestAgentAccessorsProvidersAndDirs(t *testing.T) {
	...
	if got := a.Name(); got != "antigravity" {
		t.Fatalf("Name() = %q, want antigravity", got)
	}
	if got := a.GetWorkDir(); got != workDir {
	...
	if got := a.GetMode(); got != "plan" {
		t.Fatalf("GetMode() = %q, want plan", got)
	}
	...
	if len(modes) != 3 || modes[0].Key != "default" || modes[1].Key != "yolo" || modes[2].Key != "plan" {
...
func TestNewMissingCLIAndDefaults(t *testing.T) {
	agentInf, err := New(map[string]any{
		"cmd":      "definitely-missing-antigravity-cli",
		"work_dir": "",
	})
	if err == nil {
		t.Fatalf("New returned nil error and agent %#v, want missing CLI error", agentInf)
	}
	if !strings.Contains(err.Error(), "definitely-missing-antigravity-cli") {
		t.Fatalf("missing CLI error = %v", err)
	}

	cliPath := filepath.Join(t.TempDir(), "agy")
	...
	a := agentInf.(*Agent)
	if a.workDir != "." || a.mode != "yolo" || a.cmd != cliPath {
	...
	if a.timeout != 2*time.Minute {
		t.Fatalf("timeout = %v, want 2m", a.timeout)
	}
}
