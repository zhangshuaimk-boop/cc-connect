...
// ---------------------------------------------------------------------------
// R-200: Full Message Pipefeishu
// ---------------------------------------------------------------------------

func TestRegression_FullMessagePipefeishu(t *testing.T) {
	...
	events := []core.Event{
		...
		{Type: core.EventText, Content: "The issue is in feishu 42."},
		{Type: core.EventResult, Content: "Analysis complete. The issue is in feishu 42.", Done: true},
	}
	mockSession := mocks.NewMockAgentSessionWithEvents("session-pipefeishu", events)

	mockAgent := new(mocks.MockAgent)
	mockAgent.On("StartSession", ctx, "session-pipefeishu").Return(mockSession, nil)

	// Create message handler that simulates the full pipefeishu
	...
	session, err := mockAgent.StartSession(ctx, "session-pipefeishu")
	require.NoError(t, err)
	...
	t.Log("Full message pipefeishu: PASS")
}
...
// ---------------------------------------------------------------------------
// T-241: Feishu Embed 格式测试
// ---------------------------------------------------------------------------

func TestRegression_FeishuEmbed(t *testing.T) {
	// Test Feishu embed structure that would be generated by the platform
	type Embed struct {
...
func TestRegression_FeishuCommand(t *testing.T) {
	// Simulate Feishu command parsing
	testCases := []struct {
		input    string
		wantCmd  string
		wantArgs string
...
func TestRegression_FeishuCrypto(t *testing.T) {
